export const resources = {
  es: {
    translation: {
      common: {
        back: 'Atrás',
        exit: 'Salir',
        account: 'Cuenta',
        games: 'Juegos',
        settings: 'Ajustes',
        loading: 'Cargando',
        guest: 'Invitado',
        player: 'Jugador',
        cancel: 'Cancelar',
        save: 'Guardar',
        stay: 'Quedarse',
        continue: 'Continuar',
        on: 'Activado',
        off: 'Desactivado',
        secondsShort: 's'
      },
      auth: {
        badge: 'MiniGamesWF',
        eyebrow: 'Noche social de juegos',
        title: 'Inicia sesión',
        subtitle:
          'Entra rápido como invitado o con Google, o usa tu correo y contraseña para seguir con tu cuenta.',
        quickAccessTitle: 'Entrada rápida',
        quickAccessCopy: 'El acceso como invitado entra directo al juego y funciona como un usuario real dentro de las salas.',
        continueAsGuest: 'Continuar como invitado',
        continueWithGoogle: 'Continuar con Google',
        formTitle: 'Crea tu cuenta',
        formTitleSignIn: 'Inicia sesión',
        formSubtitleSignIn: 'Usa tu correo y contraseña para entrar a tu identidad y tus salas.',
        formSubtitle: 'Usa tu nombre, correo y contraseña para seguir con tu misma identidad en móvil y web.',
        modalTitle: 'Crea tu cuenta',
        modalSubtitle: 'Regístrate con correo o usa Google para dejar vinculada tu identidad desde el inicio.',
        modalClose: 'Cerrar',
        nameLabel: 'Nombre',
        emailLabel: 'Correo',
        passwordLabel: 'Contraseña',
        divider: 'o',
        primaryCta: 'Crear cuenta',
        primaryCtaSignIn: 'Iniciar sesión',
        signInPrompt: '¿Ya tienes cuenta?',
        signInAction: 'Inicia sesión',
        signUpPrompt: '¿No tienes cuenta?',
        signUpAction: 'Crear cuenta',
        terms: 'Al continuar, aceptas nuestros Términos de servicio y Política de privacidad.',
        guestHint: 'Puedes vincular tu cuenta después desde la pestaña Cuenta.',
        googleNotReady: 'Google todavía necesita terminar de conectarse en la configuración de MiniGamesWF.',
        guestNotReady: 'El acceso como invitado todavía no está habilitado en Supabase Auth.',
        authCancelled: 'Se canceló el acceso antes de completarse.',
        authCallbackIncomplete: 'La respuesta de autenticación llegó incompleta. Revisa los redirect URLs de Google y Supabase.',
        displayNamePlaceholder: 'Cómo te verá tu grupo',
        emailPlaceholder: 'tu-correo@ejemplo.com',
        passwordPlaceholder: 'Tu contraseña',
        signIn: 'Iniciar sesión',
        signUp: 'Crear cuenta',
        supabaseMissing: 'Falta configurar la conexión de Supabase para activar el acceso real.',
        signUpSuccess: 'Cuenta creada. Si tu proyecto exige confirmación por correo, revisa tu inbox antes de entrar.'
      },
      account: {
        title: 'Cuenta',
        subtitle: 'Administra tu nombre visible y convierte tu invitado en una cuenta con correo cuando quieras.',
        profileSection: 'Perfil actual',
        displayName: 'Nombre visible',
        displayNamePlaceholder: 'Cómo quieres que te vea tu grupo',
        saveName: 'Guardar nombre',
        displayNameSaved: 'Tu nombre ya quedó actualizado.',
        displayNameRequired: 'Escribe un nombre antes de guardar.',
        username: 'Usuario',
        email: 'Correo',
        guestBadge: 'Invitado',
        accountData: 'Datos de cuenta',
        accountDataHint: 'Tu nombre visible puede cambiar cuando quieras. Tu identificador tipo guess12345 se mantiene estable para ubicarte fácil en la sala.',
        accountState: 'Estado de cuenta',
        accountStateGuest: 'Invitado',
        accountStateAuthenticated: 'Autenticado',
        linkedMethod: 'Método de acceso',
        accountDetails: 'Datos de cuenta',
        missingUsername: 'Todavía no tienes un usuario visible.',
        linkedAccounts: 'Cuenta vinculada',
        linkedAccountsEmpty: 'No hay una cuenta adicional vinculada todavía.',
        linkedAccountsHint: 'El flujo queda preparado, pero requiere credenciales reales del proveedor para completarse.',
        linkAccount: 'Vincular cuenta',
        linkedAccountsGuestHint: 'Estás jugando como invitado. Vincula una cuenta cuando quieras guardar identidad y acceso.',
        guestPrimaryLink: 'Vincular cuenta para guardar acceso',
        linkAccountSection: 'Vincular cuenta',
        linkAccountHint: 'Agrega tu correo para convertir este invitado en una cuenta permanente. Te enviaremos un correo de verificación para terminar la vinculación.',
        linkWithEmail: 'Vincular con correo',
        passwordOptional: 'Contraseña (después de verificar)',
        passwordOptionalPlaceholder: 'Puedes dejarla lista para después',
        emailLinkVerification: 'Revisa tu correo y confirma el enlace de verificación. Después podrás terminar el acceso con contraseña.',
        accountAlreadyLinked: 'Esta cuenta ya no está en modo invitado.',
        billingSection: 'Planes y pagos',
        currentPlan: 'Plan actual',
        currentPlanEmpty: 'Todavía no hay una suscripción o plan conectado.',
        billingHint: 'La estructura queda lista para billing y upgrades cuando el backend de pagos esté disponible.',
        billingCta: 'Administrar plan y pagos',
        dataSection: 'Datos disponibles',
        missingDisplayName: 'Todavía no definiste un nombre visible.',
        missingEmail: 'Tu proveedor no expuso un correo.',
        linkingRequiresSetup: 'Hace falta terminar la configuración del proveedor antes de completar la vinculación.',
        linkingGuestEntry: 'La entrada para vincular cuenta ya está prevista, pero todavía necesita el proveedor real.',
        billingNotReady: 'La gestión real de pagos y plan queda preparada, pero todavía no está conectada.',
        notConfigured: 'Supabase todavía no está configurado.',
        savingLanguage: 'Guardando idioma...'
      },
      profile: {
        avatar: 'Avatar',
        frame: 'Marco',
        avatarPicker: 'Avatar y marco',
        avatarPickerHint: 'Elige cómo te ven tus amigos y jugadores dentro de la sala.',
        avatarSaved: 'Avatar actualizado.'
      },
      avatars: {
        devil: 'Diablito',
        moreSoon: 'Más personajes próximamente.',
        joker: 'Joker',
        mask: 'Máscara',
        ghost: 'Fantasma',
        vampire: 'Vampiro',
        knight: 'Caballero',
        wizard: 'Mago',
        princess: 'Princesa',
        robot: 'Robot',
        alien: 'Alien',
        ninja: 'Ninja',
        pirate: 'Pirata'
      },
      frames: {
        plain: 'Sin marco',
        gold: 'Oro',
        crimson: 'Carmesí',
        mint: 'Menta',
        dashed: 'Discontinuo'
      },
      friends: {
        title: 'Amigos',
        subtitle: 'Agrega amigos por usuario y acepta solicitudes.',
        onlineTitle: 'Amigos online',
        activeCount: '{{count}} activos',
        invite: 'Invitar',
        emptyInvite: 'Invitar',
        empty: 'Invita a tus amigos para jugar juntos.',
        emptyHint: 'Cuando acepten tu solicitud aparecerán aquí.',
        noRequests: 'No tienes solicitudes pendientes.',
        friendsTab: 'Amigos',
        requestsTab: 'Solicitudes',
        addFriend: 'Agregar amigo',
        sendRequest: 'Enviar',
        searchPlaceholder: 'username de tu amigo',
        requestSent: 'Solicitud enviada.',
        requestAccepted: 'Solicitud aceptada.',
        requestRejected: 'Solicitud rechazada.',
        removed: 'Amigo eliminado.',
        acceptRequest: 'Aceptar',
        rejectRequest: 'Rechazar',
        removeFriend: 'Quitar amigo',
        notFound: 'No encontramos ese usuario.',
        alreadyFriends: 'Ya existe una solicitud o amistad con ese usuario.',
        selfBlocked: 'No puedes agregarte a ti mismo.',
        usernameRequired: 'Escribe un username.',
        status: {
          online: 'Online',
          offline: 'Offline',
          pending: 'Pendiente'
        }
      },
      settings: {
        title: 'Ajustes',
        subtitle: 'Controla idioma, sistema visual y tu sesión.',
        openAccount: 'Abrir cuenta',
        accountHint: 'Gestiona {{account}} y abre los datos de cuenta dentro de este panel. Estado actual: {{state}}.',
        language: 'Idioma',
        languageHint: 'El idioma cambia al instante en toda la interfaz.',
        spanish: 'Español',
        english: 'Inglés',
        theme: 'Sistema de color',
        themeHint: 'La selección queda guardada desde ahora.',
        logout: 'Cerrar sesión',
        appearanceSection: 'Diseño visual',
        appearanceCardHint: 'Elige un estilo visual completo para toda la interfaz. La selección vive dentro de un panel dedicado.',
        openAppearance: 'Abrir apariencia',
        activeAppearance: 'Activa',
        motionBackground: 'Fondo en movimiento',
        motionBackgroundHint: 'Activa un movimiento sutil en el fondo de la app.',
        themeChoices: {
          'neutral-light': { label: 'Neutro claro' },
          'neutral-dark': { label: 'Neutro oscuro' },
          green: { label: 'Verde' },
          gold: { label: 'Dorado' },
          'light-blue': { label: 'Azul claro' },
          'dark-green': { label: 'Verde oscuro' },
          'legacy-dark': { label: 'Legacy' }
        },
        sessionSection: 'Sesión',
        savingTheme: 'Guardando tema...'
      },
      navigation: {
        accountCta: 'Cuenta',
        accountTab: 'Cuenta',
        gamesTab: 'Juegos',
        settingsTab: 'Ajustes'
      },
      lobby: {
        home: 'Inicio',
        guestHeaderStatus: 'Estás en modo invitado',
        createRoom: 'Crear sala',
        joinByCode: 'Entrar por código',
        quickPlay: 'Partida rápida',
        scanQr: 'Escanear QR',
        guestStatus: 'Modo invitado',
        guestTitle: 'Entra rápido o únete con código.',
        guestSubtitle: 'Puedes entrar a una sesión en segundos. Crea tu sala o únete a la que ya abrió tu grupo.',
        noRoomStatus: 'Sin sala activa',
        noRoomTitle: 'Todavía no hay una sala abierta.',
        noRoomSubtitle: 'Crea una cuando tu grupo esté listo, o entra con un código si alguien ya empezó.',
        activeStatusActive: 'Sala activa',
        activeStatusWaiting: 'Sala en espera',
        activeTitleReady: '{{code}} ya está lista.',
        activeSubtitleHost: 'La sala ya está guardada y la lista de miembros se actualiza en vivo. Comparte el código o sigue con la configuración.',
        activeSubtitleMember: 'Tu sala sigue abierta. Vuelve a entrar y espera a que el host continúe.',
        continueRoom: 'Continuar sala',
        shareCode: 'Compartir código',
        leaveStoredRoom: 'Salir de la sala',
        leaveStoredRoomConfirmTitle: '¿Salir de la sala?',
        leaveStoredRoomConfirmMember: 'La sala seguirá abierta. Puedes volver a entrar con el código.',
        leaveStoredRoomConfirmHost: 'Eres el host. Si sales, la sala se cerrará y todos los jugadores serán expulsados.',
        roomLinkInvalid: 'Ese enlace de sala no es válido.',
        openingRoom: 'Abriendo sala {{code}}...',
        switchingRoom: 'Saliendo de la sala {{from}} y abriendo {{to}}...',
        joinAsGuest: 'Entrando a la sala {{code}} como invitado...',
        shareReady: 'El código {{code}} ya está listo para compartirse.',
        noRoomToShare: 'No hay una sala activa para compartir ahora.',
        noRoomToContinue: 'No se encontró una sala activa para continuar.',
        noRoomToResume: 'No se encontró una sala activa para retomar.',
        roomLinkNeedsAuth: 'Los enlaces de sala necesitan que el backend de acceso esté configurado primero.'
        ,
        errors: {
          roomNotFound: 'Ese código no corresponde a una sala abierta.',
          roomUnavailable: 'Esa sala ya no está disponible.',
          authRequired: 'Abre primero una sesión de invitado o de cuenta.',
          roomFull: 'Esa sala está llena en este momento.',
          backendNotConfigured: 'Las salas todavía no están configuradas en el backend.',
          backendUnreachable: 'No se pudo conectar con Supabase en este momento.',
          permissionDenied: 'Las policies de salas en Supabase todavía no permiten leer la sala recién creada. Vuelve a correr el schema actualizado.',
          noActiveRoomFallback: 'No se encontró una sala activa. Crea una o entra por código.'
        }
      },
      gameMeta: {
        names: {
          impostor: 'Impostor',
          'guess-who': 'Adivina Quién Soy',
          'faces-gestures': 'Caras y Gestos',
          trivia: 'Trivia',
          'who-said': '¿Quién dijo esto?',
          majority: 'Adivina la mayoría',
          troll: 'Troll Diferente',
          'whose-top': '¿De quién es este top?',
          'signal-drop': 'Signal Drop',
          'hot-seat': 'Hot Seat',
          'mentiroso-profesional': 'Mentiroso Profesional',
          'after-hours': 'After Hours',
          'close-call': 'Close Call'
        },
        descriptions: {
          impostor: 'Una o varias personas no conocen la palabra. La sala conversa, vota y trata de sacar a todos los impostores.',
          'guess-who': 'Ve a los demás. Adivina quién eres.',
          'faces-gestures': 'Un actor interpreta. Los demás adivinan.',
          trivia: 'Preguntas rápidas por temas.',
          'who-said': 'Frases anónimas. Adivina quién las escribió.',
          majority: 'Elige. Luego predice la mayoría.',
          troll: 'Una palabra real, un impostor y un troll.',
          'whose-top': 'Crea tu top. La sala adivina.',
          'signal-drop': 'Todos adivinan la palabra oculta a partir de pistas incompletas y confianza fingida.',
          'hot-seat': 'Una persona responde prompts rápidos mientras la sala le sube la presión.',
          'mentiroso-profesional': 'Elige la mentira más creíble bajo presión y luego defiéndela como si siempre hubiera sido verdad.',
          'after-hours': 'Los jugadores se abren paso con bluff en escenarios sociales imposibles y votan por la mejor salvada.',
          'close-call': 'La sala ordena decisiones salvajes y luego discute quién conoce mejor al grupo.'
        },
        categories: {
          'Warm Up': 'Calentamiento',
          'Social Reads': 'Lectura social',
          Bluffing: 'Bluff',
          'Social Deduction': 'Deducción social'
        },
        energies: {
          'Warm-up': 'Ligero',
          Social: 'Social',
          Chaotic: 'Caótico'
        }
      },
      chooseGames: {
        title: 'Elegir mini juegos',
        subtitle: 'Elige los juegos para esta sala.',
        onlyModeTitle: 'Modo activo',
        onlyModeCopy: 'La sala usa únicamente Impostor mientras cerramos el resto del sistema multijugador.',
        selected: 'Seleccionado',
        tapToAdd: 'Listo para activar',
        keepSelected: 'Mantener Impostor',
        selectImpostor: 'Usar Impostor',
        selectAtLeastOne: 'Elige al menos un juego para continuar.',
        save: 'Guardar juegos'
      },
      gamesCatalog: {
        title: 'Juegos',
        catalogTitle: 'Catálogo de juegos',
        subtitle: 'Elige los minijuegos para tu grupo.',
        selectedCount: '{{count}} juegos seleccionados',
        futureSlot: 'Próximamente',
        impostorHowToPlayTitle: 'Cómo funciona Impostor',
        impostorHowToPlayIntro: 'Todos reciben una palabra secreta, excepto los impostores.',
        impostorHowToPlayStepOne: '1. La sala conversa y hace preguntas sin revelar demasiado.',
        impostorHowToPlayStepTwo: '2. Cuando el grupo sospecha de alguien, vota para expulsarlo.',
        impostorHowToPlayStepThree: '3. Los inocentes ganan si descubren a los impostores; los impostores ganan si sobreviven.',
        howToPlayTitle: 'Cómo funciona Impostor',
        howToPlayIntro: 'Todos reciben una palabra secreta, excepto los impostores.',
        howToPlayStepOne: '1. La sala conversa y hace preguntas sin revelar demasiado.',
        howToPlayStepTwo: '2. Cuando el grupo sospecha de alguien, vota para expulsarlo.',
        howToPlayStepThree: '3. Los inocentes ganan si descubren a los impostores; los impostores ganan si sobreviven.',
        guessWhoHowToPlayTitle: 'Cómo funciona',
        guessWhoHowToPlayIntro: 'Ves a los demás. El tuyo queda oculto.',
        guessWhoHowToPlayStepOne: '1. Pregunta al grupo.',
        guessWhoHowToPlayStepTwo: '2. Escribe tu respuesta.',
        guessWhoHowToPlayStepThree: '3. Tienes 2 intentos.',
        facesGesturesHowToPlayTitle: 'Cómo funciona',
        facesGesturesHowToPlayIntro: 'Un actor interpreta. Los demás adivinan.',
        facesGesturesHowToPlayStepOne: '1. Solo el actor ve el personaje.',
        facesGesturesHowToPlayStepTwo: '2. La sala escribe respuestas.',
        facesGesturesHowToPlayStepThree: '3. Ganan al acertar antes del tiempo.',
        triviaHowToPlayTitle: 'Cómo funciona',
        triviaHowToPlayIntro: 'Responde preguntas antes del tiempo.',
        triviaHowToPlayStepOne: '1. El host elige temas.',
        triviaHowToPlayStepTwo: '2. Todos responden cada pregunta.',
        triviaHowToPlayStepThree: '3. El ranking da puntos.',
        whoSaidHowToPlayTitle: 'Cómo funciona',
        whoSaidHowToPlayIntro: 'Escribe una frase. La sala adivina.',
        whoSaidHowToPlayStepOne: '1. Todos escriben.',
        whoSaidHowToPlayStepTwo: '2. Se muestra anónimo.',
        whoSaidHowToPlayStepThree: '3. Adivina quién fue.',
        majorityHowToPlayTitle: 'Cómo funciona',
        majorityHowToPlayIntro: 'Elige y predice la mayoría.',
        majorityHowToPlayStepOne: '1. Todos eligen.',
        majorityHowToPlayStepTwo: '2. Predice la mayoría.',
        majorityHowToPlayStepThree: '3. Empates cuentan.',
        trollHowToPlayTitle: 'Cómo funciona Troll Diferente',
        trollHowToPlayIntro: 'Una palabra real, un impostor sin palabra y un troll con palabra equivocada.',
        trollHowToPlayStepOne: '1. Cada jugador recibe una palabra: real, impostor sin nada, o troll con engaño.',
        trollHowToPlayStepTwo: '2. La sala discute y vota a quien crean impostor.',
        trollHowToPlayStepThree: '3. El troll gana si los inocentes echan a un troll por error.',
        whoseTopHowToPlayTitle: 'Cómo funciona ¿De quién es este top?',
        whoseTopHowToPlayIntro: 'Cada jugador crea un top personal y la sala intenta adivinar de quién es.',
        whoseTopHowToPlayStepOne: '1. Todos ordenan su top en secreto.',
        whoseTopHowToPlayStepTwo: '2. La sala ve un top anónimo y vota por su autor.',
        whoseTopHowToPlayStepThree: '3. Ganas puntos por adivinar y por confundir al grupo.'
      },
      roomSettings: {
        title: 'Configuración del juego',
        subtitle: 'Ajusta el modo seleccionado. Por ahora la sala usa Impostor y aquí defines sus reglas antes de iniciar la ronda.',
        modeTitle: 'Modo de partida',
        modeTournament: 'Torneo',
        modeSingle: 'Juego único',
        singleRoundCount: 'Número de rondas',
        modeTournamentHint: 'Varios juegos seguidos con marcador acumulado.',
        modeSingleHint: 'Un solo juego repetido varias rondas.',
        modalTitle: 'Configuración - {{game}}',
        modalSubtitle: 'Cambia las reglas de la partida actual sin salir de la sala.',
        profileTitle: 'Perfil de la sala de hoy',
        chatOn: 'Chat activado',
        chatOff: 'Chat desactivado',
        summary:
          'Una sala con {{impostorCount}} impostor(es), temática {{theme}} y una votación de {{turnSeconds}} segundos.',
        impostorCount: 'Número de impostores',
        impostorCountValue_one: '{{count}} impostor',
        impostorCountValue_other: '{{count}} impostores',
        theme: 'Temática',
        themeHint: 'Cuando empiece la ronda, la palabra secreta saldrá al azar desde la carpeta de esta temática.',
        maxPlayers: 'Máximo de jugadores',
        rounds: 'Rondas',
        turnTimer: 'Temporizador por turno',
        turnTimerOptions: {
          none: 'Sin tiempo',
          fiveMinutes: '5 min'
        },
        missBehavior: 'Qué pasa si no sale un impostor',
        missBehaviorHint: 'Decide si la partida termina de inmediato o si continúa otra ronda cuando expulsan a un inocente.',
        missBehaviorOptions: {
          repeat: 'Continuar otra ronda',
          end: 'Terminar la partida'
        },
        balanceRule: 'Victoria por balance',
        balanceRuleHint: 'Los impostores ganan cuando quedan tantos impostores como inocentes.',
        balanceRuleDisabledHint: 'Esta regla solo aplica si la partida continúa después de expulsar a un inocente.',
        settings: 'Ajustes',
        gameSettingsTitle: 'Configuración de {{game}}',
        noSettings: 'Sin ajustes',
        guessWhoCategory: 'Categoría',
        guessWhoCategoryOptions: {
          popular: 'Populares',
          'movies-series': 'Películas y series'
        },
        triviaQuestionCount: 'Preguntas',
        triviaTopics: 'Temas',
        triviaTopicOptions: {
          famosos: 'Famosos',
          f1: 'F1',
          'cultura-general': 'Cultura general',
          marcas: 'Marcas',
          'personajes-ficticios': 'Personajes ficticios',
          objetos: 'Objetos'
        },
        whoSaidTopic: 'Tema',
        whoSaidWriteTime: 'Escribir',
        whoSaidGuessTime: 'Adivinar',
        whoSaidTopicOptions: {
          comida: 'Comida',
          famosos: 'Famosos',
          'peliculas-series': 'Películas/series',
          musica: 'Música',
          deportes: 'Deportes',
          libre: 'Libre'
        },
        majorityCategory: 'Categoría',
        majorityRounds: 'Rondas',
        majorityAnswerTime: 'Elige',
        majorityPredictionTime: 'Adivina',
        majorityCategoryOptions: {
          comida: 'Comida',
          gustos: 'Gustos',
          'peliculas-series': 'Películas/series',
          musica: 'Música',
          random: 'Random',
          amigos: 'Amigos'
        },
        trollDiscussionTime: 'Discusión',
        trollVotingTime: 'Votar',
        whoseTopCategory: 'Categoría',
        whoseTopSize: 'Top',
        whoseTopCreateTime: 'Crear',
        whoseTopGuessTime: 'Adivinar',
        whoseTopCategoryOptions: {
          'mejores-actores': 'Mejores actores',
          'mejores-comidas': 'Mejores comidas',
          'mejores-peliculas': 'Mejores películas',
          'mejores-videojuegos': 'Mejores videojuegos',
          'mejores-cantantes': 'Mejores cantantes',
          'mejores-marcas': 'Mejores marcas'
        },
        privacy: 'Privacidad',
        roomVibe: 'Ritmo de la sala',
        format: 'Formato',
        roomChat: 'Chat de sala',
        save: 'Guardar ajustes de sala',
        privacyOptions: {
          'Invite only': 'Solo por invitación',
          'Friends of friends': 'Amigos de amigos'
        },
        vibeOptions: {
          Balanced: 'Balanceado',
          Fast: 'Rápido',
          Talkative: 'Conversado'
        },
        formatOptions: {
          Casual: 'Casual',
          Competitive: 'Competitivo'
        },
        themeOptions: {
          animals: 'Animales',
          countries: 'Países y capitales',
          objects: 'Objetos',
          'famous-people': 'Actores famosos',
          'football-players': 'Jugadores de fútbol',
          'movies-series': 'Películas y series',
          youtubers: 'Youtubers',
          basketball: 'Basquetbol',
          f1: 'F1',
          singers: 'Cantantes',
          'cartoons-fictional': 'Personajes de caricaturas o ciencia ficción',
          'world-foods': 'Comidas del mundo'
        }
      },
      joinRoom: {
        title: 'Entrar a sala',
        subtitle: 'Ingresa el código que compartió el host. Si la sala sigue abierta, entrarás directo.',
        roomCode: 'Código de sala',
        helper: 'Los códigos son cortos, privados y tienen 5 caracteres. Si no funciona, pide al host el código más reciente.',
        joinParty: 'Entrar a la sala',
        scanInstead: 'Escanear QR en su lugar',
        invalidCode: 'Ingresa un código válido de 5 caracteres.'
      },
      scanRoom: {
        title: 'Escanear QR',
        subtitle: 'Apunta la cámara al QR de MiniGamesWF y entra a la sala al instante.',
        checkingCamera: 'Revisando acceso a la cámara...',
        cameraNeeded: 'Se necesita acceso a la cámara para escanear el QR. También puedes entrar manualmente con el código.',
        allowCamera: 'Permitir cámara',
        invalidQr: 'Este QR no corresponde a una sala válida de MiniGamesWF.',
        detected: 'QR detectado: {{code}}',
        joining: 'Entrando a la sala {{code}}...',
        scanAgain: 'Escanear otra vez',
        joinWithCode: 'Entrar con código'
      },
      room: {
        title: 'Sala privada',
        roomCode: 'Código de sala',
        statusActive: 'En partida',
        statusFinished: 'Terminada',
        statusWaiting: 'En espera',
        syncLive: 'Sync en vivo',
        syncConnecting: 'Sincronizando...',
        syncReconnect: 'Reconectar',
        syncIdle: 'En pausa',
        hostLine: '{{host}} está hosteando · {{count}} miembros activos',
        hostUnavailable: 'Host no disponible',
        shareCode: 'Compartir código',
        returnToGame: 'Volver al juego',
        continueToGame: 'Continuar al juego',
        startGame: 'Iniciar juego',
        qrTitle: 'Escanea para entrar al instante',
        fallbackCode: 'Código alterno: {{code}}',
        members: 'Miembros',
        activeCount: '{{count}} activos',
        you: 'Tú',
        memberHost: 'Host',
        memberJoined: 'Dentro',
        memberAway: 'Ausente',
        gameModes: 'Modos de juego',
        chooseGame: 'Elegir',
        selected: 'Seleccionado',
        startsFirst: 'Primero',
        removeMember: 'Expulsar',
        memberRemoved: 'El miembro salió de la sala.',
        memberMissing: 'Ese miembro ya no está dentro de la sala.',
        removeHostBlocked: 'No puedes expulsarte como host de la sala.',
        roundStarting: 'Preparando la ronda compartida...',
        minimumPlayersRequired: 'Necesitas al menos 3 jugadores activos para iniciar Impostor.',
        minimumPlayersRequiredGuessWho: 'Necesitas al menos 2 jugadores activos para iniciar.',
        minimumPlayersRequiredFacesGestures: 'Necesitas al menos 2 jugadores activos para iniciar.',
        minimumPlayersRequiredTrivia: 'Necesitas al menos 1 jugador activo para iniciar.',
        gameUnavailable: 'Este juego todavía no tiene partida disponible.',
        impostorHint: 'La sala entra directo a la ronda compartida de Impostor en cuanto el host la inicie.',
        noModeHost: 'Elige el primer modo antes de empezar la sala.',
        noModeMember: 'El host todavía no ha seleccionado el siguiente modo.',
        hostOnlyContinue: 'Solo el host puede continuar el flujo de la sala.',
        gameSetup: 'Configuración del juego',
        configureGame: 'Ajustar',
        configureRoom: 'Configurar sala',
        leaveRoom: 'Salir de la sala',
        closeRoom: 'Cerrar sala',
        leaveConfirmMemberTitle: '¿Estás seguro de que quieres salir de la sala?',
        leaveConfirmMemberBody: 'Saldrás de esta sala y dejarás de aparecer como miembro activo.',
        leaveConfirmMemberAction: 'Salir de la sala',
        leaveConfirmHostTitle: '¿Estás seguro de que quieres deshacer la sala?',
        leaveConfirmHostBody: 'La sala se cerrará para todos los miembros y el grupo volverá al menú principal.',
        leaveConfirmHostAction: 'Cerrar sala',
        roomLeftNotice: 'Saliste de la sala.',
        roomClosedNotice: 'La sala se cerró correctamente.',
        roomExpired: 'La sala anterior caducó por inactividad.',
        idleWarning: 'La sala se cerrará en 1 minuto si no hay actividad. Toca cualquier botón para mantenerla activa.',
        privacyLine: 'Privacidad: {{value}}',
        maxPlayersLine: 'Máximo de jugadores: {{value}}',
        roundsLine: 'Rondas: {{value}}',
        impostorCountLine: 'Impostores: {{value}}',
        themeLine: 'Temática: {{value}}',
        guessWhoCategoryLine: 'Categoría: {{value}}',
        guessWhoAttemptsLine: 'Intentos: 2',
        triviaQuestionsLine: 'Preguntas: {{value}}',
        whoSaidTopicLine: 'Tema: {{value}}',
        whoSaidWriteLine: 'Escribir: {{value}}',
        whoSaidGuessLine: 'Adivinar: {{value}}',
        minimumPlayersRequiredWhoSaid: 'Se necesitan al menos 2 jugadores para ¿Quién dijo esto?.',
        majorityCategoryLine: 'Categoría: {{value}}',
        majorityAnswerLine: 'Elige: {{value}}',
        majorityPredictionLine: 'Adivina: {{value}}',
        minimumPlayersRequiredMajority: 'Se necesitan al menos 2 jugadores para Adivina la mayoría.',
        trollDiscussionLine: 'Discusión: {{value}}',
        trollVotingLine: 'Votar: {{value}}',
        minimumPlayersRequiredTroll: 'Se necesitan al menos 4 jugadores para Troll Diferente.',
        whoseTopCategoryLine: 'Categoría: {{value}}',
        whoseTopSizeLine: 'Top {{value}}',
        whoseTopCreateLine: 'Crear: {{value}}',
        whoseTopGuessLine: 'Adivinar: {{value}}',
        minimumPlayersRequiredWhoseTop: 'Se necesitan al menos 2 jugadores para ¿De quién es este top?.',
        themeUnavailable: 'La temática elegida ya no está disponible.',
        turnTimerLine: 'Temporizador: {{value}}',
        missBehaviorLine: {
          repeat: 'Si fallan el voto: se juega otra ronda',
          end: 'Si fallan el voto: termina la partida'
        },
        balanceRuleOn: 'Balance impostor vs inocentes: activado',
        balanceRuleOff: 'Balance impostor vs inocentes: desactivado',
        modeLine: 'Formato: {{value}}',
        flowLine: 'Ritmo: {{value}}',
        chatLine: 'Chat: {{value}}',
        chatEnabled: 'Activado',
        chatDisabled: 'Desactivado'
      },
      gameplay: {
        impostorTitle: 'Impostor',
        impostorSubtitle: 'La sala entra directo al reveal privado. El host ya dejó lista la temática y el número de impostores.',
        privateReveal: 'Tu reveal privado',
        privateRevealCopy: 'Mira tu rol sin enseñarlo. Después conversa normal con la sala hasta que llegue la votación.',
        impostorWord: 'IMPOSTOR',
        impostorTag: 'Tu rol',
        secretWordTag: 'Tu palabra',
        impostorHelper: 'No ves la palabra. Escucha a los demás, adapta tu discurso y trata de no destacar.',
        civilianHelper: 'Tu palabra secreta es {{word}}. Habla con naturalidad sin regalarla demasiado.',
        roundSetup: 'Configuración de esta ronda',
        roundNumber: 'Ronda {{current}} de {{total}}',
        roundSimple: 'Ronda {{current}}',
        totalPlayers: '{{count}} jugadores en la ronda',
        impostorCount_one: '{{count}} impostor en juego',
        impostorCount_other: '{{count}} impostores en juego',
        themeSelected: 'Temática elegida: {{value}}',
        roundsConfigured: '{{count}} rondas configuradas para la sala',
        waitingForHostTitle: 'Esperando al host',
        waitingForHostCopy: 'La ronda compartida aparecerá aquí en cuanto el host la inicie para toda la sala.',
        playersTitle: 'Jugadores en esta ronda',
        youLabel: 'Tú',
        playerWaiting: 'Esperando la conversación',
        playerActive: 'Sigue dentro de la ronda',
        playerEliminated: 'Ya fue eliminado en esta ronda',
        voteHint: 'Tócalo para votar su expulsión',
        voted: 'Votado',
        voteSection: 'Votación',
        votePrompt: 'Cuando el host abra la votación, podrás elegir a quién sacar de la ronda.',
        voteOpen: 'La votación está abierta. Elige a quién expulsar.',
        voteOpenNoTimer: 'La votación está abierta y terminará cuando todos hayan votado.',
        votePrimary: 'Votar',
        voteRegistered: 'Tu voto ya quedó registrado.',
        voteRegisteredFor: 'Tu voto actual va contra {{player}}.',
        voteModalTitle: 'Elegir jugador',
        voteModalSubtitle: 'Selecciona a quién quieres expulsar y confirma el voto.',
        selectedVote: 'Elegido',
        confirmVote: 'Confirmar voto',
        voteTimer: '{{time}}',
        voteCount_one: '{{count}} voto',
        voteCount_other: '{{count}} votos',
        voteClosed: 'Votación cerrada',
        openVoting: 'Abrir votación',
        expelVote: 'Expulsar por voto',
        expelled: 'Expulsado',
        eliminated: 'Eliminado',
        impostorReveal: 'Era impostor',
        notImpostor: 'No era impostor',
        voteSuccessTitle: 'Encontraron al impostor',
        voteFailTitle: 'La sala se equivocó',
        voteResultLine: '{{player}} fue eliminado por la votación de la sala.',
        votePending: 'Todavía no hay un voto final cerrado.',
        roundMissing: 'La ronda compartida ya no está disponible.',
        roundNotReady: 'La siguiente ronda todavía no está lista.',
        voteNotOpen: 'La votación todavía no está abierta.',
        voteTargetMissing: 'Ese jugador ya no está disponible para votar.',
        voteTargetGone: 'Ese jugador ya salió de la ronda.',
        revealedImpostor: '{{player}} sí era impostor.',
        revealedCivilian: '{{player}} no era impostor.',
        remainingImpostors_one: 'Todavía queda {{count}} impostor dentro.',
        remainingImpostors_other: 'Todavía quedan {{count}} impostores dentro.',
        roomFinishedTimeout_one: 'Quedó {{count}} impostor dentro, pero la sala se cerró al terminar el tiempo.',
        roomFinishedTimeout_other: 'Quedaron {{count}} impostores dentro, pero la sala se cerró al terminar el tiempo.',
        nextRound: 'Siguiente ronda',
        nextRoundStarting: 'Preparando la siguiente ronda...',
        nextRoundAutoHost: 'Todos los impostores salieron. La siguiente ronda arranca enseguida.',
        nextRoundAuto: 'La sala sigue. La siguiente ronda arranca enseguida.',
        playAgain: 'Otra partida',
        backToRoom: 'Volver a la sala',
        waitingForHostDecision: 'Esperando a que el host decida qué sigue.',
        returningToRoom: 'Volviendo a la sala...',
        finishMatch: 'Partida terminada',
        matchFinished: 'Ya se jugaron todas las rondas configuradas para esta sala.',
        roomFinishedClosing: 'La sala terminó. Volviendo al menú principal...',
        missedImpostorEnd: 'No expulsaron a ningún impostor y la configuración indica que la partida termina aquí.',
        balanceWin_one: 'Queda {{count}} impostor y el número ya igualó a los inocentes. Los impostores ganan.',
        balanceWin_other: 'Quedan {{count}} impostores y el número ya igualó a los inocentes. Los impostores ganan.',
        newWord: 'Nueva palabra',
        finishRound: 'Terminar ronda',
        genericTitle: 'Partida',
        genericSubtitle: 'Este modo todavía usa la superficie genérica mientras cerramos el flujo completo.'
      },
      guessWho: {
        title: 'Adivina Quién Soy',
        subtitle: 'Mira a los demás. Adivina el tuyo.',
        yourCharacter: 'Tu personaje',
        hidden: 'Oculto',
        attempts: 'Intentos: {{count}}',
        placeholder: 'Escribe tu respuesta',
        guess: 'Adivinar',
        correct: 'Correcto',
        noAttempts: 'Sin intentos',
        players: 'Jugadores',
        tryAgain: 'Intenta otra vez'
      },
      facesGestures: {
        title: 'Caras y Gestos',
        actorSubtitle: 'Interpreta sin decirlo.',
        guesserSubtitle: 'Mira y adivina.',
        actorTitle: 'Actúa',
        guessTitle: 'Adivina',
        actor: 'Actor',
        yourPrompt: 'Tu personaje',
        hidden: 'Oculto',
        actorHelper: 'Solo tú ves esto.',
        placeholder: 'Tu respuesta',
        guess: 'Adivinar',
        correctWithAnswer: '¡Correcto! Adivinaste: {{value}}',
        tryAgain: 'No es ese personaje. Sigue intentando.',
        completed: 'Correcto',
        waiting: 'Intentando',
        finished: 'Ronda terminada',
        timer: '{{value}}s',
        players: 'Jugadores',
        actorCannotGuess: 'El actor no adivina esta ronda.'
      },
      tournament: {
        title: 'Resultados',
        subtitle: 'Puntaje final del torneo.',
        endTitle: '¡Torneo completado!',
        endSubtitle: 'Marcador final del torneo.',
        scoreboard: 'Marcador',
        winner: 'Ganador',
        points: '{{count}} pts',
        backToRoom: 'Volver a la sala',
        playAgain: 'Jugar otra vez',
        restartTournament: 'Nuevo torneo',
        closeRoom: 'Cerrar sala',
        nextGame: 'Siguiente juego',
        modeTournament: 'Torneo',
        modeSingle: 'Juego único',
        roundCount: 'Número de rondas',
        singleProgress: 'Ronda {{current}} de {{total}}',
        results: 'Resultados',
        roundDone: 'Ronda lista',
        place: {
          1: '1er lugar',
          2: '2do lugar',
          3: '3er lugar'
        }
      },
      trivia: {
        title: 'Trivia',
        subtitle: 'Responde antes del tiempo.',
        finished: 'Ranking final.',
        questionCount: '{{current}}/{{total}}',
        timer: '{{value}}s',
        placeholder: 'Tu respuesta',
        submit: 'Responder',
        correct: 'Correcto',
        wrong: 'No era esa',
        next: 'Siguiente',
        players: 'Jugadores',
        pending: 'Pendiente',
        answered: 'Listo',
        ranking: 'Ranking',
        correctCount: '{{count}} correctas',
        pointsEarned: '+{{count}} pts'
      },
      whoSaid: {
        title: '¿Quién dijo esto?',
        write: 'Escribe tu frase',
        placeholder: 'Tu frase',
        send: 'Enviar',
        ready: 'Listo',
        pending: 'Pendiente',
        waiting: 'Esperando',
        guessTitle: '¿Quién lo dijo?',
        guess: 'Adivinar',
        correct: 'Correcto',
        guessed: 'Respuesta enviada',
        reveal: 'Era de...',
        next: 'Siguiente',
        done: 'Final',
        timer: '{{value}}s',
        phraseCount: '{{current}}/{{total}}',
        players: 'Jugadores',
        author: 'Autor',
        authorWait: 'Es tu frase.'
      },
      majority: {
        title: 'Adivina la mayoría',
        choose: 'Elige',
        predict: 'Adivina la mayoría',
        guess: 'Adivinar',
        majority: 'Mayoría',
        correct: 'Correcto',
        wrong: 'No fue',
        result: 'Resultado',
        next: 'Siguiente',
        done: 'Final',
        ready: 'Listo',
        pending: 'Pendiente',
        players: 'Jugadores',
        timer: '{{value}}s',
        roundCount: '{{current}}/{{total}}',
        noResult: 'Sin resultado'
      },
      troll: {
        title: 'Troll Diferente',
        discuss: 'Discusión',
        vote: 'Votar',
        result: 'Resultado',
        yourRole: 'Tu rol',
        yourWord: 'Tu palabra',
        noWord: '???',
        hidden: 'Oculto',
        eliminated: 'Eliminado',
        players: 'Jugadores',
        ready: 'Listo',
        done: 'Final',
        next: 'Siguiente',
        timer: '{{value}}s',
        roundCount: '{{current}}/{{total}}',
        roles: {
          innocent: 'Inocente',
          impostor: 'Impostor',
          troll: 'Troll'
        },
        outcomes: {
          troll_eliminated: 'Cayó el troll.',
          impostor_eliminated: 'Cayó el impostor.',
          innocent_eliminated: 'Cayó un inocente.',
          continue: 'Sigue la ronda.'
        }
      },
      whoseTop: {
        title: '¿De quién es este top?',
        create: 'Crea tu top',
        yourTop: 'Tu top',
        send: 'Enviar',
        guessTitle: '¿De quién es?',
        guess: 'Adivinar',
        correct: 'Correcto',
        guessed: 'Respuesta enviada',
        reveal: 'Era de...',
        next: 'Siguiente',
        done: 'Final',
        ready: 'Listo',
        pending: 'Pendiente',
        players: 'Jugadores',
        author: 'Autor',
        authorWait: 'Es tu top.',
        timer: '{{value}}s',
        topCount: '{{current}}/{{total}}'
      }
    }
  },
  en: {
    translation: {
      common: {
        back: 'Back',
        exit: 'Exit',
        account: 'Account',
        games: 'Games',
        settings: 'Settings',
        loading: 'Loading',
        guest: 'Guest',
        player: 'Player',
        cancel: 'Cancel',
        save: 'Save',
        stay: 'Stay',
        continue: 'Continue',
        on: 'On',
        off: 'Off',
        secondsShort: 's'
      },
      auth: {
        badge: 'MiniGamesWF',
        eyebrow: 'Social game night',
        title: 'Sign in',
        subtitle:
          'Jump in as a guest or create your account to keep your identity, rooms, and settings across mobile and web.',
        quickAccessTitle: 'Fast entry',
        quickAccessCopy: 'Guest entry is the quickest path and still works as a real room identity.',
        continueAsGuest: 'Continue as Guest',
        continueWithGoogle: 'Continue with Google',
        formTitle: 'Create your account',
        formTitleSignIn: 'Sign in',
        formSubtitleSignIn: 'Use your email and password to enter with your identity and active rooms.',
        formSubtitle: 'Use your name, email, and password to keep the same identity across mobile and web.',
        modalTitle: 'Create your account',
        modalSubtitle: 'Register with email or use Google to link your identity from the start.',
        modalClose: 'Close',
        nameLabel: 'Name',
        emailLabel: 'Email',
        passwordLabel: 'Password',
        divider: 'or',
        primaryCta: 'Create Account',
        primaryCtaSignIn: 'Sign In',
        signInPrompt: 'Already have an account?',
        signInAction: 'Sign In Now',
        signUpPrompt: 'Need an account?',
        signUpAction: 'Create one',
        terms: 'By continuing, you agree to our Terms of Service and Privacy Policy.',
        guestHint: 'You can link your account later from the Account tab.',
        googleNotReady: 'Google sign-in still needs provider configuration for this project.',
        guestNotReady: 'Guest access is not enabled in Supabase Auth yet.',
        authCancelled: 'The sign-in flow was cancelled before it finished.',
        authCallbackIncomplete: 'The auth callback came back incomplete. Check your Google and Supabase redirect URLs.',
        displayNamePlaceholder: 'How your group will see you',
        emailPlaceholder: 'your-email@example.com',
        passwordPlaceholder: 'Your password',
        signIn: 'Sign in',
        signUp: 'Create account',
        supabaseMissing: 'The Supabase connection is missing. Configure it to enable real sign-in.',
        signUpSuccess: 'Account created. If your project requires email confirmation, check your inbox before signing in.'
      },
      account: {
        title: 'Account',
        subtitle: 'Manage your visible name and convert this guest into an email account whenever you want.',
        profileSection: 'Current profile',
        displayName: 'Display name',
        displayNamePlaceholder: 'How you want your group to see you',
        saveName: 'Save name',
        displayNameSaved: 'Your name was updated.',
        displayNameRequired: 'Enter a name before saving.',
        username: 'Username',
        email: 'Email',
        guestBadge: 'Guest',
        accountData: 'Account data',
        accountDataHint: 'Your visible name can change whenever you want. Your guess12345-style handle stays stable so people can still identify you in the room.',
        accountState: 'Account state',
        accountStateGuest: 'Guest',
        accountStateAuthenticated: 'Authenticated',
        linkedMethod: 'Sign-in method',
        accountDetails: 'Account details',
        missingUsername: 'You do not have a visible username yet.',
        linkedAccounts: 'Linked account',
        linkedAccountsEmpty: 'No additional linked account yet.',
        linkedAccountsHint: 'The flow is scaffolded, but it still needs real provider credentials to complete.',
        linkAccount: 'Link account',
        linkedAccountsGuestHint: 'You are playing as a guest. Link an account whenever you want to save identity and access.',
        guestPrimaryLink: 'Link account to save access',
        linkAccountSection: 'Link account',
        linkAccountHint: 'Add your email to convert this guest into a permanent account. We will send a verification email to finish the link.',
        linkWithEmail: 'Link with email',
        passwordOptional: 'Password (after verification)',
        passwordOptionalPlaceholder: 'You can leave this ready for later',
        emailLinkVerification: 'Check your email and confirm the verification link. After that you can finish password access.',
        accountAlreadyLinked: 'This account is no longer a guest account.',
        billingSection: 'Plans and payments',
        currentPlan: 'Current plan',
        currentPlanEmpty: 'No connected subscription or plan yet.',
        billingHint: 'The UI structure is ready for billing and upgrades once the payments backend exists.',
        billingCta: 'Manage plan and billing',
        dataSection: 'Available data',
        missingDisplayName: 'You have not set a display name yet.',
        missingEmail: 'Your provider did not expose an email.',
        linkingRequiresSetup: 'You still need to finish the provider setup before account linking can complete.',
        linkingGuestEntry: 'The account-linking entry point is ready, but it still needs the real provider setup.',
        billingNotReady: 'Real billing and plan management are scaffolded, but not connected yet.',
        notConfigured: 'Supabase is not configured yet.',
        savingLanguage: 'Saving language...'
      },
      profile: {
        avatar: 'Avatar',
        frame: 'Frame',
        avatarPicker: 'Avatar and frame',
        avatarPickerHint: 'Choose how friends and room players see you.',
        avatarSaved: 'Avatar updated.'
      },
      avatars: {
        devil: 'Devil',
        moreSoon: 'More characters coming soon.',
        joker: 'Joker',
        mask: 'Mask',
        ghost: 'Ghost',
        vampire: 'Vampire',
        knight: 'Knight',
        wizard: 'Wizard',
        princess: 'Princess',
        robot: 'Robot',
        alien: 'Alien',
        ninja: 'Ninja',
        pirate: 'Pirate'
      },
      frames: {
        plain: 'No frame',
        gold: 'Gold',
        crimson: 'Crimson',
        mint: 'Mint',
        dashed: 'Dashed'
      },
      friends: {
        title: 'Friends',
        subtitle: 'Add friends by username and accept requests.',
        onlineTitle: 'Online friends',
        activeCount: '{{count}} active',
        invite: 'Invite',
        emptyInvite: 'Invite',
        empty: 'Invite your friends to play together.',
        emptyHint: 'Accepted friends will appear here.',
        noRequests: 'No pending requests.',
        friendsTab: 'Friends',
        requestsTab: 'Requests',
        addFriend: 'Add friend',
        sendRequest: 'Send',
        searchPlaceholder: "friend's username",
        requestSent: 'Request sent.',
        requestAccepted: 'Request accepted.',
        requestRejected: 'Request rejected.',
        removed: 'Friend removed.',
        acceptRequest: 'Accept',
        rejectRequest: 'Reject',
        removeFriend: 'Remove friend',
        notFound: 'We could not find that user.',
        alreadyFriends: 'A request or friendship already exists with that user.',
        selfBlocked: 'You cannot add yourself.',
        usernameRequired: 'Enter a username.',
        status: {
          online: 'Online',
          offline: 'Offline',
          pending: 'Pending'
        }
      },
      settings: {
        title: 'Settings',
        subtitle: 'Control language, visual system, and your session.',
        openAccount: 'Open account',
        accountHint: 'Manage {{account}} and open account details inside this panel. Current state: {{state}}.',
        language: 'Language',
        languageHint: 'Language updates instantly across the interface.',
        spanish: 'Spanish',
        english: 'English',
        theme: 'Color system',
        themeHint: 'The choice is already saved.',
        logout: 'Log out',
        appearanceSection: 'Visual design',
        appearanceCardHint: 'Choose a complete visual style for the whole interface. Selection lives inside a dedicated panel.',
        openAppearance: 'Open appearance',
        activeAppearance: 'Active',
        motionBackground: 'Motion background',
        motionBackgroundHint: 'Enable subtle background movement across the app.',
        themeChoices: {
          'neutral-light': { label: 'Neutral light' },
          'neutral-dark': { label: 'Neutral dark' },
          green: { label: 'Green' },
          gold: { label: 'Gold' },
          'light-blue': { label: 'Light blue' },
          'dark-green': { label: 'Dark green' },
          'legacy-dark': { label: 'Legacy' }
        },
        sessionSection: 'Session',
        savingTheme: 'Saving theme...'
      },
      navigation: {
        accountCta: 'Account',
        accountTab: 'Account',
        gamesTab: 'Games',
        settingsTab: 'Settings'
      },
      lobby: {
        home: 'Home',
        guestHeaderStatus: 'You are in guest mode',
        createRoom: 'Create room',
        joinByCode: 'Join by code',
        quickPlay: 'Quick play',
        scanQr: 'Scan QR',
        guestStatus: 'Guest mode',
        guestTitle: 'Start fast or jump into a room by code.',
        guestSubtitle: 'You can get into a session in seconds. Create your own room or join the one your friends already opened.',
        noRoomStatus: 'No active room',
        noRoomTitle: 'No room open yet.',
        noRoomSubtitle: 'Create one when your group is ready, or jump in with a room code if someone already started.',
        activeStatusActive: 'Active room',
        activeStatusWaiting: 'Waiting room',
        activeTitleReady: '{{code}} is ready.',
        activeSubtitleHost: 'The room is persisted and the member list is live. Share the code or continue setup.',
        activeSubtitleMember: 'Your room is still open. Jump back in and wait for the host to continue.',
        continueRoom: 'Continue room',
        shareCode: 'Share code',
        leaveStoredRoom: 'Leave room',
        leaveStoredRoomConfirmTitle: 'Leave room?',
        leaveStoredRoomConfirmMember: 'The room will stay open. You can join again with the code.',
        leaveStoredRoomConfirmHost: 'You are the host. Leaving closes the room and removes every player.',
        roomLinkInvalid: 'That room link is not valid.',
        openingRoom: 'Opening room {{code}}...',
        switchingRoom: 'Leaving room {{from}} and opening {{to}}...',
        joinAsGuest: 'Joining room {{code}} as guest...',
        shareReady: 'Room code {{code}} ready to share.',
        noRoomToShare: 'No active room to share right now.',
        noRoomToContinue: 'No active room found to continue.',
        noRoomToResume: 'No active room found to resume.',
        roomLinkNeedsAuth: 'Room links need backend auth configured first.',
        errors: {
          roomNotFound: 'That code does not match any open party.',
          roomUnavailable: 'That room is no longer available.',
          authRequired: 'Open a guest or account session first.',
          roomFull: 'That room is full right now.',
          backendNotConfigured: 'Rooms are not configured in the backend yet.',
          backendUnreachable: 'The Supabase backend could not be reached right now.',
          permissionDenied: 'Room policies in Supabase are still blocking reads for the newly created room. Re-run the updated schema.',
          noActiveRoomFallback: 'No active room found. Create one or join by code.'
        }
      },
      gameMeta: {
        names: {
          impostor: 'Impostor',
          'guess-who': 'Guess Who I Am',
          'faces-gestures': 'Faces and Gestures',
          trivia: 'Trivia',
          'who-said': 'Who Said This?',
          majority: 'Guess the Majority',
          troll: 'Different Troll',
          'whose-top': 'Whose Top Is This?',
          'signal-drop': 'Signal Drop',
          'hot-seat': 'Hot Seat',
          'mentiroso-profesional': 'Mentiroso Profesional',
          'after-hours': 'After Hours',
          'close-call': 'Close Call'
        },
        descriptions: {
          impostor: 'One or more players do not know the secret word. The room talks, votes, and tries to remove every impostor.',
          'guess-who': 'See everyone else. Guess yourself.',
          'faces-gestures': 'One actor performs. Everyone else guesses.',
          trivia: 'Fast questions by topic.',
          'who-said': 'Anonymous phrases. Guess who wrote them.',
          majority: 'Choose. Then predict the majority.',
          troll: 'One real word, one impostor, one troll.',
          'whose-top': 'Make your top. The room guesses.',
          'signal-drop': 'Everyone guesses the hidden word from half-finished clues and fake confidence.',
          'hot-seat': 'One player answers fast prompts while the room piles on the pressure.',
          'mentiroso-profesional': 'Pick the most believable lie under pressure, then defend it like you meant it all along.',
          'after-hours': 'Players bluff their way through impossible social scenarios and vote on the smoothest save.',
          'close-call': 'The room ranks wild choices, then fights over who really knows the group best.'
        },
        categories: {
          'Warm Up': 'Warm Up',
          'Social Reads': 'Social Reads',
          Bluffing: 'Bluffing',
          'Social Deduction': 'Social Deduction'
        },
        energies: {
          'Warm-up': 'Warm-up',
          Social: 'Social',
          Chaotic: 'Chaotic'
        }
      },
      chooseGames: {
        title: 'Choose mini games',
        subtitle: 'Choose the games for this room.',
        onlyModeTitle: 'Active mode',
        onlyModeCopy: 'The room only uses Impostor while the rest of the multiplayer system is being finished.',
        selected: 'Selected',
        tapToAdd: 'Ready to activate',
        keepSelected: 'Keep Impostor',
        selectImpostor: 'Use Impostor',
        selectAtLeastOne: 'Choose at least one game to continue.',
        save: 'Save games'
      },
      gamesCatalog: {
        title: 'Games',
        catalogTitle: 'Game catalog',
        subtitle: 'Choose minigames for your group.',
        selectedCount: '{{count}} games selected',
        futureSlot: 'Coming soon',
        impostorHowToPlayTitle: 'How Impostor Works',
        impostorHowToPlayIntro: 'Everyone gets a secret word, except the impostors.',
        impostorHowToPlayStepOne: '1. The room talks and asks questions without revealing too much.',
        impostorHowToPlayStepTwo: '2. When the group suspects someone, everyone votes to remove them.',
        impostorHowToPlayStepThree: '3. Innocents win by finding the impostors; impostors win by surviving.',
        howToPlayTitle: 'How Impostor Works',
        howToPlayIntro: 'Everyone gets a secret word, except the impostors.',
        howToPlayStepOne: '1. The room talks and asks questions without revealing too much.',
        howToPlayStepTwo: '2. When the group suspects someone, everyone votes to remove them.',
        howToPlayStepThree: '3. Innocents win by finding the impostors; impostors win by surviving.',
        guessWhoHowToPlayTitle: 'How it works',
        guessWhoHowToPlayIntro: 'You see everyone else. Yours stays hidden.',
        guessWhoHowToPlayStepOne: '1. Ask the group.',
        guessWhoHowToPlayStepTwo: '2. Type your answer.',
        guessWhoHowToPlayStepThree: '3. You have 2 tries.',
        facesGesturesHowToPlayTitle: 'How it works',
        facesGesturesHowToPlayIntro: 'One actor performs. Everyone else guesses.',
        facesGesturesHowToPlayStepOne: '1. Only the actor sees the prompt.',
        facesGesturesHowToPlayStepTwo: '2. The room types guesses.',
        facesGesturesHowToPlayStepThree: '3. Solve before time runs out.',
        triviaHowToPlayTitle: 'How it works',
        triviaHowToPlayIntro: 'Answer questions before time runs out.',
        triviaHowToPlayStepOne: '1. Host picks topics.',
        triviaHowToPlayStepTwo: '2. Everyone answers each question.',
        triviaHowToPlayStepThree: '3. Ranking gives points.',
        whoSaidHowToPlayTitle: 'How it works',
        whoSaidHowToPlayIntro: 'Write a phrase. The room guesses.',
        whoSaidHowToPlayStepOne: '1. Everyone writes.',
        whoSaidHowToPlayStepTwo: '2. It shows anonymous.',
        whoSaidHowToPlayStepThree: '3. Guess who.',
        majorityHowToPlayTitle: 'How it works',
        majorityHowToPlayIntro: 'Choose and predict the majority.',
        majorityHowToPlayStepOne: '1. Everyone chooses.',
        majorityHowToPlayStepTwo: '2. Predict the majority.',
        majorityHowToPlayStepThree: '3. Ties count.',
        trollHowToPlayTitle: 'How Troll Diferente Works',
        trollHowToPlayIntro: 'One real word, one impostor with no word, and one troll with the wrong word.',
        trollHowToPlayStepOne: '1. Each player receives a word: real, no-word impostor, or misleading troll.',
        trollHowToPlayStepTwo: '2. The room discusses and votes for whoever seems like the impostor.',
        trollHowToPlayStepThree: '3. The troll wins if innocents remove a troll by mistake.',
        whoseTopHowToPlayTitle: 'How Whose Top Works',
        whoseTopHowToPlayIntro: 'Each player creates a personal top list and the room guesses whose it is.',
        whoseTopHowToPlayStepOne: '1. Everyone secretly orders their top list.',
        whoseTopHowToPlayStepTwo: '2. The room sees an anonymous top and votes for its author.',
        whoseTopHowToPlayStepThree: '3. Score by guessing correctly and by fooling the room.'
      },
      roomSettings: {
        title: 'Game settings',
        subtitle: 'Adjust the selected mode. For now the room runs Impostor and this is where you define its rules before the round starts.',
        modeTitle: 'Match mode',
        modeTournament: 'Tournament',
        modeSingle: 'Single game',
        singleRoundCount: 'Round count',
        modeTournamentHint: 'Several games in sequence with a cumulative score.',
        modeSingleHint: 'One selected game repeated for several rounds.',
        modalTitle: 'Settings - {{game}}',
        modalSubtitle: 'Change the current match rules without leaving the room.',
        profileTitle: "Tonight's room profile",
        chatOn: 'Chat on',
        chatOff: 'Chat off',
        summary:
          'A room with {{impostorCount}} impostor(s), a {{theme}} theme, and a {{turnSeconds}}-second voting timer.',
        impostorCount: 'Number of impostors',
        impostorCountValue_one: '{{count}} impostor',
        impostorCountValue_other: '{{count}} impostors',
        theme: 'Theme',
        themeHint: 'When the round starts, the secret word will be picked at random from this theme folder.',
        maxPlayers: 'Max players',
        rounds: 'Rounds',
        turnTimer: 'Turn timer',
        turnTimerOptions: {
          none: 'No timer',
          fiveMinutes: '5 min'
        },
        missBehavior: 'If no impostor is expelled',
        missBehaviorHint: 'Choose whether the match ends immediately or continues into another round after a wrong vote.',
        missBehaviorOptions: {
          repeat: 'Continue to another round',
          end: 'End the match'
        },
        balanceRule: 'Balance win rule',
        balanceRuleHint: 'Impostors win when the number of impostors matches the number of innocents left.',
        balanceRuleDisabledHint: 'This rule only applies if the match continues after a wrong expulsion.',
        settings: 'Settings',
        gameSettingsTitle: '{{game}} settings',
        noSettings: 'No settings',
        guessWhoCategory: 'Category',
        guessWhoCategoryOptions: {
          popular: 'Popular',
          'movies-series': 'Movies and series'
        },
        triviaQuestionCount: 'Questions',
        triviaTopics: 'Topics',
        triviaTopicOptions: {
          famosos: 'Famous people',
          f1: 'F1',
          'cultura-general': 'General knowledge',
          marcas: 'Brands',
          'personajes-ficticios': 'Fictional characters',
          objetos: 'Objects'
        },
        whoSaidTopic: 'Topic',
        whoSaidWriteTime: 'Write',
        whoSaidGuessTime: 'Guess',
        whoSaidTopicOptions: {
          comida: 'Food',
          famosos: 'Famous people',
          'peliculas-series': 'Movies/series',
          musica: 'Music',
          deportes: 'Sports',
          libre: 'Free'
        },
        majorityCategory: 'Category',
        majorityRounds: 'Rounds',
        majorityAnswerTime: 'Choose',
        majorityPredictionTime: 'Guess',
        majorityCategoryOptions: {
          comida: 'Food',
          gustos: 'Taste',
          'peliculas-series': 'Movies/series',
          musica: 'Music',
          random: 'Random',
          amigos: 'Friends'
        },
        trollDiscussionTime: 'Discussion',
        trollVotingTime: 'Vote',
        whoseTopCategory: 'Category',
        whoseTopSize: 'Top',
        whoseTopCreateTime: 'Create',
        whoseTopGuessTime: 'Guess',
        whoseTopCategoryOptions: {
          'mejores-actores': 'Best actors',
          'mejores-comidas': 'Best foods',
          'mejores-peliculas': 'Best movies',
          'mejores-videojuegos': 'Best videogames',
          'mejores-cantantes': 'Best singers',
          'mejores-marcas': 'Best brands'
        },
        privacy: 'Privacy',
        roomVibe: 'Room vibe',
        format: 'Format',
        roomChat: 'Room chat',
        save: 'Save room settings',
        privacyOptions: {
          'Invite only': 'Invite only',
          'Friends of friends': 'Friends of friends'
        },
        vibeOptions: {
          Balanced: 'Balanced',
          Fast: 'Fast',
          Talkative: 'Talkative'
        },
        formatOptions: {
          Casual: 'Casual',
          Competitive: 'Competitive'
        },
        themeOptions: {
          animals: 'Animals',
          countries: 'Countries and capitals',
          objects: 'Objects',
          'famous-people': 'Famous actors',
          'football-players': 'Football players',
          'movies-series': 'Movies and series',
          youtubers: 'YouTubers',
          basketball: 'Basketball',
          f1: 'F1',
          singers: 'Singers',
          'cartoons-fictional': 'Cartoon or sci-fi characters',
          'world-foods': 'World foods'
        }
      },
      joinRoom: {
        title: 'Join room',
        subtitle: 'Enter the code your host shared. If the room is still open, you will drop straight into the party.',
        roomCode: 'Room code',
        helper: 'Codes are short, private, and 5 characters long. Ask the host to share the latest code if this one does not work.',
        joinParty: 'Join party',
        scanInstead: 'Scan QR instead',
        invalidCode: 'Enter a valid 5-character room code.'
      },
      scanRoom: {
        title: 'Scan QR',
        subtitle: 'Point the camera at a MiniGamesWF room QR and join the party instantly.',
        checkingCamera: 'Checking camera access...',
        cameraNeeded: 'Camera access is required to scan a room QR. You can still join manually with the room code.',
        allowCamera: 'Allow camera',
        invalidQr: 'This QR is not a valid MiniGamesWF room link.',
        detected: 'QR detected: {{code}}',
        joining: 'Joining room {{code}}...',
        scanAgain: 'Scan again',
        joinWithCode: 'Join with code'
      },
      room: {
        title: 'Party room',
        roomCode: 'Room code',
        statusActive: 'In session',
        statusFinished: 'Finished',
        statusWaiting: 'Waiting',
        syncLive: 'Live sync',
        syncConnecting: 'Syncing...',
        syncReconnect: 'Reconnect',
        syncIdle: 'Idle',
        hostLine: '{{host}} is hosting · {{count}} active members',
        hostUnavailable: 'Host unavailable',
        shareCode: 'Share code',
        returnToGame: 'Return to game',
        continueToGame: 'Continue to game',
        startGame: 'Start game',
        qrTitle: 'Scan to join instantly',
        fallbackCode: 'Fallback code: {{code}}',
        members: 'Members',
        activeCount: '{{count}} active',
        you: 'You',
        memberHost: 'Host',
        memberJoined: 'Joined',
        memberAway: 'Away',
        gameModes: 'Game modes',
        chooseGame: 'Choose',
        selected: 'Selected',
        startsFirst: 'First',
        removeMember: 'Remove',
        memberRemoved: 'That member was removed from the room.',
        memberMissing: 'That member is no longer in the room.',
        removeHostBlocked: 'The host cannot remove themselves from the room.',
        roundStarting: 'Preparing the shared round...',
        minimumPlayersRequired: 'You need at least 3 active players to start Impostor.',
        minimumPlayersRequiredGuessWho: 'You need at least 2 active players to start.',
        minimumPlayersRequiredFacesGestures: 'You need at least 2 active players to start.',
        minimumPlayersRequiredTrivia: 'You need at least 1 active player to start.',
        gameUnavailable: 'This game is not playable yet.',
        impostorHint: 'The room jumps straight into the shared Impostor round as soon as the host starts it.',
        noModeHost: 'Choose the first mode before starting the party.',
        noModeMember: 'The host has not selected the next mode yet.',
        hostOnlyContinue: 'Only the host can continue the party flow.',
        gameSetup: 'Game settings',
        configureGame: 'Adjust',
        configureRoom: 'Configure room',
        leaveRoom: 'Leave room',
        closeRoom: 'Close room',
        leaveConfirmMemberTitle: 'Are you sure you want to leave the room?',
        leaveConfirmMemberBody: 'You will leave this room and stop appearing as an active member.',
        leaveConfirmMemberAction: 'Leave room',
        leaveConfirmHostTitle: 'Are you sure you want to close this room?',
        leaveConfirmHostBody: 'The room will be closed for all members and everyone will return to the main menu.',
        leaveConfirmHostAction: 'Close room',
        roomLeftNotice: 'You left the room.',
        roomClosedNotice: 'The room was closed successfully.',
        roomExpired: 'The previous room expired due to inactivity.',
        idleWarning: 'The room will close in 1 minute without activity. Tap any button to keep it alive.',
        privacyLine: 'Privacy: {{value}}',
        maxPlayersLine: 'Max players: {{value}}',
        roundsLine: 'Rounds: {{value}}',
        impostorCountLine: 'Impostors: {{value}}',
        themeLine: 'Theme: {{value}}',
        guessWhoCategoryLine: 'Category: {{value}}',
        guessWhoAttemptsLine: 'Attempts: 2',
        triviaQuestionsLine: 'Questions: {{value}}',
        whoSaidTopicLine: 'Topic: {{value}}',
        whoSaidWriteLine: 'Write: {{value}}',
        whoSaidGuessLine: 'Guess: {{value}}',
        minimumPlayersRequiredWhoSaid: 'Who Said This? needs at least 2 players.',
        majorityCategoryLine: 'Category: {{value}}',
        majorityAnswerLine: 'Choose: {{value}}',
        majorityPredictionLine: 'Guess: {{value}}',
        minimumPlayersRequiredMajority: 'Guess the Majority needs at least 2 players.',
        trollDiscussionLine: 'Discussion: {{value}}',
        trollVotingLine: 'Vote: {{value}}',
        minimumPlayersRequiredTroll: 'Different Troll needs at least 4 players.',
        whoseTopCategoryLine: 'Category: {{value}}',
        whoseTopSizeLine: 'Top {{value}}',
        whoseTopCreateLine: 'Create: {{value}}',
        whoseTopGuessLine: 'Guess: {{value}}',
        minimumPlayersRequiredWhoseTop: 'Whose Top Is This? needs at least 2 players.',
        themeUnavailable: 'The selected theme is no longer available.',
        turnTimerLine: 'Turn timer: {{value}}',
        missBehaviorLine: {
          repeat: 'If the vote fails: play another round',
          end: 'If the vote fails: end the match'
        },
        balanceRuleOn: 'Impostor vs innocent balance rule: on',
        balanceRuleOff: 'Impostor vs innocent balance rule: off',
        modeLine: 'Mode: {{value}}',
        flowLine: 'Flow: {{value}}',
        chatLine: 'Chat: {{value}}',
        chatEnabled: 'On',
        chatDisabled: 'Off'
      },
      gameplay: {
        impostorTitle: 'Impostor',
        impostorSubtitle: 'The room goes straight into the private reveal. The host already set the theme and impostor count.',
        privateReveal: 'Your private reveal',
        privateRevealCopy: 'Check your role privately. Then talk naturally with the room until voting starts.',
        impostorWord: 'IMPOSTOR',
        impostorTag: 'Your role',
        secretWordTag: 'Your word',
        impostorHelper: 'You do not see the word. Listen to the others, adapt your answers, and try not to stand out.',
        civilianHelper: 'Your secret word is {{word}}. Speak naturally without giving it away too easily.',
        roundSetup: 'Round setup',
        roundNumber: 'Round {{current}} of {{total}}',
        totalPlayers: '{{count}} players in this round',
        impostorCount_one: '{{count}} impostor in play',
        impostorCount_other: '{{count}} impostors in play',
        themeSelected: 'Selected theme: {{value}}',
        roundsConfigured: '{{count}} rounds configured for the room',
        waitingForHostTitle: 'Waiting for the host',
        waitingForHostCopy: 'The shared round will appear here as soon as the host starts it for the whole room.',
        playersTitle: 'Players in this round',
        youLabel: 'You',
        playerWaiting: 'Waiting for the conversation',
        playerActive: 'Still in this round',
        playerEliminated: 'Already eliminated this round',
        voteHint: 'Tap to vote them out',
        voted: 'Voted',
        voteSection: 'Voting',
        votePrompt: 'Once the host opens voting, you can choose who gets expelled from the round.',
        voteOpen: 'Voting is open. Choose who to expel.',
        voteOpenNoTimer: 'Voting is open and will end once everyone has voted.',
        votePrimary: 'Vote',
        voteRegistered: 'Your vote has been registered.',
        voteRegisteredFor: 'Your current vote is for {{player}}.',
        voteModalTitle: 'Choose a player',
        voteModalSubtitle: 'Pick who you want to expel and confirm the vote.',
        selectedVote: 'Selected',
        confirmVote: 'Confirm vote',
        voteTimer: '{{time}}',
        voteCount_one: '{{count}} vote',
        voteCount_other: '{{count}} votes',
        voteClosed: 'Voting closed',
        openVoting: 'Open voting',
        expelVote: 'Expel by vote',
        expelled: 'Expelled',
        eliminated: 'Eliminated',
        impostorReveal: 'Was the impostor',
        notImpostor: 'Was not the impostor',
        voteSuccessTitle: 'The impostor was found',
        voteFailTitle: 'The room got it wrong',
        voteResultLine: '{{player}} was eliminated by the room vote.',
        votePending: 'There is no locked vote yet.',
        roundMissing: 'The shared round is no longer available.',
        roundNotReady: 'The next round is not ready yet.',
        voteNotOpen: 'Voting is not open yet.',
        voteTargetMissing: 'That player is no longer available to vote.',
        voteTargetGone: 'That player is already out of the round.',
        revealedImpostor: '{{player}} really was an impostor.',
        revealedCivilian: '{{player}} was not an impostor.',
        remainingImpostors_one: '{{count}} impostor is still inside.',
        remainingImpostors_other: '{{count}} impostors are still inside.',
        roomFinishedTimeout_one: '{{count}} impostor remained, but the room closed when the timer ended.',
        roomFinishedTimeout_other: '{{count}} impostors remained, but the room closed when the timer ended.',
        nextRound: 'Next round',
        nextRoundStarting: 'Preparing the next round...',
        nextRoundAutoHost: 'Every impostor is out. The next round is about to start.',
        nextRoundAuto: 'The match continues. The next round is about to start.',
        playAgain: 'Play again',
        backToRoom: 'Back to room',
        waitingForHostDecision: 'Waiting for the host to decide what happens next.',
        returningToRoom: 'Returning to the room...',
        finishMatch: 'Match finished',
        matchFinished: 'All configured rounds have already been played for this room.',
        roomFinishedClosing: 'The room ended. Returning to the main menu...',
        missedImpostorEnd: 'No impostor was expelled and the configuration says the match ends here.',
        balanceWin_one: '{{count}} impostor remains and the count now matches the innocents. The impostors win.',
        balanceWin_other: '{{count}} impostors remain and the count now matches the innocents. The impostors win.',
        newWord: 'New word',
        finishRound: 'Finish round',
        genericTitle: 'Gameplay',
        genericSubtitle: 'This mode still uses the generic play surface while the full flow is being finished.'
      },
      guessWho: {
        title: 'Guess Who I Am',
        subtitle: 'See the others. Guess yours.',
        yourCharacter: 'Your character',
        hidden: 'Hidden',
        attempts: 'Attempts: {{count}}',
        placeholder: 'Type your answer',
        guess: 'Guess',
        correct: 'Correct',
        noAttempts: 'No tries',
        players: 'Players',
        tryAgain: 'Try again'
      },
      facesGestures: {
        title: 'Faces and Gestures',
        actorSubtitle: 'Act it out silently.',
        guesserSubtitle: 'Watch and guess.',
        actorTitle: 'Act',
        guessTitle: 'Guess',
        actor: 'Actor',
        yourPrompt: 'Your character',
        hidden: 'Hidden',
        actorHelper: 'Only you can see this.',
        placeholder: 'Your answer',
        guess: 'Guess',
        correctWithAnswer: 'Correct! You guessed: {{value}}',
        tryAgain: 'That is not it. Keep trying.',
        completed: 'Correct',
        waiting: 'Trying',
        finished: 'Round finished',
        timer: '{{value}}s',
        players: 'Players',
        actorCannotGuess: 'The actor does not guess this round.'
      },
      tournament: {
        title: 'Results',
        subtitle: 'Final tournament score.',
        endTitle: 'Tournament complete!',
        endSubtitle: 'Final tournament scoreboard.',
        scoreboard: 'Scoreboard',
        winner: 'Winner',
        points: '{{count}} pts',
        backToRoom: 'Back to room',
        playAgain: 'Play again',
        restartTournament: 'New tournament',
        closeRoom: 'Close room',
        nextGame: 'Next game',
        modeTournament: 'Tournament',
        modeSingle: 'Single game',
        roundCount: 'Round count',
        singleProgress: 'Round {{current}} of {{total}}',
        results: 'Results',
        roundDone: 'Round done',
        place: {
          1: '1st place',
          2: '2nd place',
          3: '3rd place'
        }
      },
      trivia: {
        title: 'Trivia',
        subtitle: 'Answer before time runs out.',
        finished: 'Final ranking.',
        questionCount: '{{current}}/{{total}}',
        timer: '{{value}}s',
        placeholder: 'Your answer',
        submit: 'Submit',
        correct: 'Correct',
        wrong: 'Not that one',
        next: 'Next',
        players: 'Players',
        pending: 'Pending',
        answered: 'Ready',
        ranking: 'Ranking',
        correctCount: '{{count}} correct',
        pointsEarned: '+{{count}} pts'
      },
      whoSaid: {
        title: 'Who Said This?',
        write: 'Write your phrase',
        placeholder: 'Your phrase',
        send: 'Send',
        ready: 'Ready',
        pending: 'Pending',
        waiting: 'Waiting',
        guessTitle: 'Who said it?',
        guess: 'Guess',
        correct: 'Correct',
        guessed: 'Guess sent',
        reveal: 'It was...',
        next: 'Next',
        done: 'Done',
        timer: '{{value}}s',
        phraseCount: '{{current}}/{{total}}',
        players: 'Players',
        author: 'Author',
        authorWait: 'This one is yours.'
      },
      majority: {
        title: 'Guess the Majority',
        choose: 'Choose',
        predict: 'Guess the majority',
        guess: 'Guess',
        majority: 'Majority',
        correct: 'Correct',
        wrong: 'Not it',
        result: 'Result',
        next: 'Next',
        done: 'Done',
        ready: 'Ready',
        pending: 'Pending',
        players: 'Players',
        timer: '{{value}}s',
        roundCount: '{{current}}/{{total}}',
        noResult: 'No result'
      },
      troll: {
        title: 'Different Troll',
        discuss: 'Discuss',
        vote: 'Vote',
        result: 'Result',
        yourRole: 'Your role',
        yourWord: 'Your word',
        noWord: '???',
        hidden: 'Hidden',
        eliminated: 'Eliminated',
        players: 'Players',
        ready: 'Ready',
        done: 'Done',
        next: 'Next',
        timer: '{{value}}s',
        roundCount: '{{current}}/{{total}}',
        roles: {
          innocent: 'Innocent',
          impostor: 'Impostor',
          troll: 'Troll'
        },
        outcomes: {
          troll_eliminated: 'The troll was eliminated.',
          impostor_eliminated: 'The impostor was eliminated.',
          innocent_eliminated: 'An innocent was eliminated.',
          continue: 'Round continues.'
        }
      },
      whoseTop: {
        title: 'Whose Top Is This?',
        create: 'Create your top',
        yourTop: 'Your top',
        send: 'Send',
        guessTitle: 'Whose is it?',
        guess: 'Guess',
        correct: 'Correct',
        guessed: 'Guess sent',
        reveal: 'It was...',
        next: 'Next',
        done: 'Done',
        ready: 'Ready',
        pending: 'Pending',
        players: 'Players',
        author: 'Author',
        authorWait: 'This is yours.',
        timer: '{{value}}s',
        topCount: '{{current}}/{{total}}'
      }
    }
  }
} as const;
